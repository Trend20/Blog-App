...
const SinglePost = () => {
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`http://localhost:5500/api/posts/${postItem._id}`, {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:5500/api/posts/${postItem._id}`, {
  ...
  return (
    <div className="singlePost">
      <div className="singlePostWrapper">
        {postItem.photo && (
          <img className="singlePostImg" src={PF + postItem.photo} alt="" />
        )}
        {updateMode ? (
          <input
            type="text"
